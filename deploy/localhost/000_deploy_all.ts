...
const run: DeployFunction = async (hre) => {
  ...
  await execute('CollateralRatioPolicy', {from: creator, log: true}, 'setOracleDollar', oracleDollar.address);
  ...
  await execute('Treasury', {from: creator, log: true}, 'setOracleCollateral', oracleCollateral.address);
};
